...
# Load the most recent raw transcript (generated by assembly_ai.py and saved in transcripts/raw/)
...
# Get the base filename from the raw transcript
raw_transcript_filename = os.path.basename(latest_transcript)
...
messages = [
    ...
    },
    {
        "role": "user",
                "content": f"""
                Here's the transcript (remember to confirm first, then wait for the "part 1 instructions")...
...
# Initialize cleaned_transcript
cleaned_transcript = ""

# # Initialize total_length and processed_length to track progress
# total_length = len(raw_transcript)

# Loop until the transcript is fully processed
while True:
    ...
    # assistant_response = response.content[0].text if isinstance(
    #     response.content, list) else response.content
    # cleaned_transcript += assistant_response

    messages.append({"role": "assistant", "content": response.content[0].text})

    # # Update progress
    # progress = (len(cleaned_transcript) / total_length) * 100
    ...
    # if "</cleaned_transcript>" in assistant_response:
    #     break

    # If not finished yet, add a message to continue
    # messages.append({"role": "assistant", "content": assistant_response})
    messages.append(
        {"role": "user", "content": "Continue... (If you haven't finished reformatting the entire transcript content yet, then keep modifying the same markdown-formatted rich text, picking back up where you left off. Once you've finished reformatting the ENTIRE transcript, lemme know by saying \"[end transcript]\" at the very end.)"})

# # Remove the <cleaned_transcript> and </cleaned_transcript> tags
# cleaned_transcript = cleaned_transcript.replace(
#     "<cleaned_transcript>", "").replace("</cleaned_transcript>", "").strip()

print("Final reformatted transcript:", messages[-1]["content"])

# # Print and copy the cleaned transcript to the clipboard
# print(cleaned_transcript)
# pyperclip.copy(cleaned_transcript)

# # After reformatting the transcript, save it to a file
# output_directory = "/Users/bradleykoch/Coding/Projects/arno7/transcripts/cleaned"
# os.makedirs(output_directory, exist_ok=True)
# # Extract the base filename from the raw transcript
# base_filename = os.path.basename(latest_transcript)
# # Replace "Raw Transcript" with "Cleaned Transcript" in the base filename
# output_filename = base_filename.replace(
#     "Raw Transcript", "Cleaned Transcript")
# # Define the output path
# output_path = os.path.join(output_directory, output_filename)

# # Write the cleaned transcript to the output file
...
# # Print the completion message (with the clickable link and timestamp)
